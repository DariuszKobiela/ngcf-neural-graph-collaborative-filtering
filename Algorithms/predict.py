...
df = pd.read_csv('data/' + dataset + '_test.txt',
                 delimiter='\t', names=['user', 'item', 'count'])
watched_training_log = pd.read_csv(
    'data/' + dataset + '_train.txt', delimiter='\t', names=['user_id', 'item_id', 'count'])

common_user = list(set(df['user'].unique()) & set(
    watched_training_log['user_id'].unique()))
...
def rank(user):
    ...
    watch = watched_training_log[watched_training_log['user_id']
                                 == user]['item_id'].unique()
